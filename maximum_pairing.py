...
def maximum_pairing(ids, W):
    ...
    W: dict((id1, id2): weight)
        ...
        (eg, assure that id1 < id2 for  all entries, though this is not required)
        Ok for W to include edges from ids not present in ids.
    ...
    y = LpVariable.dicts(
        "pair",
        [(i, j) for i in ids for j in ids if W.get((i, j)) is not None],
        cat="Binary",
    )
    # maximize: total edge weight
    prob += lpSum(
        [W[(i, j)] * y[(i, j)] for i in ids for j in ids if W.get((i, j)) is not None]
    )
    # constraint: one match for each id
    for i in ids:
        prob += (
            lpSum(y[(i, j)] for j in ids if W.get((i, j)) is not None)
            + lpSum(y[(j, i)] for j in ids if W.get((j, i)) is not None)
            == 1
        )
    prob.solve()
    result = [
        ...
        for i in ids
        for j in ids
        if y.get((i, j)) is not None and y[(i, j)].varValue == 1
    ]
    return result
